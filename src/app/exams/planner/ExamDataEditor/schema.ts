import { normalizeToMidnight } from "@/utils/date";
import z from "zod";
...
export const schema = z
  .object({
    ...
    room: z.string().regex(/^(LH|KH|SC)\s?(Dept|\d{1,2}-\d{3})$/, {
      message:
        "รูปแบบห้องต้องเป็น LH หรือ KH เช่น 'LH3-305', 'SC9-406' หรือ 'LH Dept'",
    }),
  ...
  .transform((x) => ({ ...x, date: normalizeToMidnight(x.date) }));
